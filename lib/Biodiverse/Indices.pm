...
sub get_calculation_metadata_as_wiki {
    ...
    foreach my $text (@header) {
        ...
        $text =~ s/\b([A-Z][a-z]+[A-Z][a-z]+)\b/!$1/
          ;                     #  escape any wiki page confusion, e.g. PhyloCom
        $text = "*$text*";
    }
...
#= q{<wiki:gadget url="http://mathml-gadget.googlecode.com/svn/trunk/mathml-gadget.xml" border="0" up_content="};
  ...
  BY_TYPE:
    foreach my $type ( sort keys %calculation_hash ) {
        ...
        $type_text =~ s/\b([A-Z][a-z]+[A-Z][a-z]+)\b/!$1/
          ;    #  escape any wiki page confusion, e.g. PhyloCom
        $html .= "==$type_text==";
      ...
      BY_NAME:    #  loop through the names
        foreach
          my $ref ( sort { $a->get_name cmp $b->get_name } values %$type_ref )
        {
...
#$html .= "<p><b>Module:</b>   $ref->{source_module}</p>\n";  #  not supported yet
            ...
            croak 'Formula is not an array'
              if defined $formula and (!is_arrayref($formula));
            ...
            foreach my $index ( sort keys %{ $ref->get_indices } ) {
                ...
                if ( 1 and $formula and ( is_arrayref($formula) ) ) {
                ...
                push @line,
                  $ref->get_index_is_cluster_metric($index)
                  ? "cluster metric"
                  : $SPACE;
                push @line,
                     $ref->get_index_uses_nbr_lists($index)
                  || $ref->get_uses_nbr_lists
                  || $SPACE;
...
sub parse_dependencies_for_calc {
    ...
    my $calcs =
      [ $args{calculation} ];    # array is a leftover - need to correct it

    my $nbr_list_count =
      $args{nbr_list_count} || $self->get_param('NBR_LIST_COUNT') || 1;
...
"[INDICES] WARNING: Insufficient neighbour lists for $calc. "
            ...
            foreach my $index ( keys %{ $metadata->get_indices } ) {
                my $index_uses_nbr_lists =
                  $metadata->get_index_uses_nbr_lists($index);
                ...
                foreach my $required_arg ( sort @$reqd_args_a ) {
                    ...
                    if ( !$is_defined ) {
                        Biodiverse::Indices::MissingRequiredArguments->throw(
                            error =>
                              "[INDICES] WARNING: $calc missing required "
...
sub get_valid_calculations {
  ...
  CALC:
    ...
    if ( scalar @removed ) {
        say
          "[INDICES] The following calcs are not valid and have been removed:\n"
          . join q{ }, @removed;
    }

    my %aggregated_calc_lists =
      $self->aggregate_calc_lists_by_type( calc_hash => \%valid_calcs, );

    my %aggregated_deps_per_calc =
      $self->get_deps_per_calc_by_type( calc_hash => \%valid_calcs, );
...
sub get_required_args {
    ...
    foreach my $calculations ( keys %$list ) {
        my $ref = $self->get_metadata( sub => $calculations );
        if ( my $reqd_ags = $ref->get_required_args )
        {    #  make it a hash if it not already
            if ( is_arrayref($reqd_ags) ) {
                my %hash;
...
sub get_required_args_including_dependencies {
    ...
    foreach my $calc (@calculations) {
        ...
        if ( my $reqd_args = $ref->get_required_args )
        {    #  make it a hash if it not already
            if ( is_arrayref($reqd_args) ) {
                my @r = @$reqd_args;
                if ( $args{no_regexp} )
                { #  dirty hack to skip regexps since these are currently only for locals
...
sub get_valid_region_grower_indices {
    ...
    foreach my $calc ( keys %$list ) {
      ...
      INDEX:
        foreach my $index ( keys %{ $meta->get_indices } ) {
            next INDEX
              if !$meta->get_index_is_lumper($index)
...
sub get_zscore_indices {
    ...
    foreach my $calculations ( keys %$list ) {
        ...
        INDEX:
        foreach my $index ( keys %{ $meta->get_indices } ) {
            next INDEX if !$meta->get_index_is_zscore($index);
            $indices{$index} = $meta->get_index_description($index);
        }
    }
...
sub index_is_zscore {
    ...
    my $zscore_hash = $self->get_zscore_indices;

    return !!$zscore_hash->{$args{index} // ''};
}

sub get_ratio_indices {
    my $self = shift;
    my %args = @_;
    my $list = $args{calculations} || $self->get_calculations_as_flat_hash;
...
sub index_is_ratio {
    ...
    my $hash = $self->get_ratio_indices;
