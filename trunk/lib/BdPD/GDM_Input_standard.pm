...
#                   - text to add to the filename for feedback
    ...
    if (!exists $$SPM{output_file_prefix}) {
        if ($$SPM{dist_measure} eq "phylo_sorenson") {
            $SPM -> set_param ("output_file_prefix",'phylo_dist_');}
        elsif ($args{dist_measure} eq "sorenson") {
    ...
    foreach my $tree_iterate (0 .. ($tree_count -1)) {
        ...
        for my $group_sets (keys %grouplists) {
            ...
            print "\nAbout to send $args{dist_measure} results to $$SPM{directory}\\$$SPM{output_file_name} \n";
            ...
            print $result_file_handle "x0,y0,x1,y1,dist".$geog_dist_output.$regions_output."\n";
...
sub prepare_regions {
    ...
    ##---------------------------------------------    
    ##---------------------------------------------
    ##  TEMP CODE SECTION 16 April 2010           #
    ##                                            #
    ##  This section stops the program execution, and outputs species richness for each region instead
    #
    #my $region_richness;
    #my $result_file_handle = $$self{result_file_handle};
    #
    #print "\nA temporary code section in GDM_Input::prepare_regions is outputting the region richness values instead of site pairs.  Comment out this code for normal site pair generation.\n";
    #
    ## write file header row
    #print $result_file_handle "region, richness\n";
    #
    #foreach my $region (keys %region_stats){
    #    $region_richness = $region_stats{$region}{label_count};
    #    print $result_file_handle "$region, $region_richness\n";
    #};
    #
    #my $closed_ok = close($result_file_handle);
    #
    #exit;
    #
    ##  END TEMP CODE SECTION 16 April 2010       #
    ##---------------------------------------------
    ##---------------------------------------------    
            ...
            while (($j < $j_count) and (! $region_completed)) {
                ...
                if ($measure == 2) {  # phylo_sorenson
                    ...
                    if (($phylo_abc{PHYLO_A} + $phylo_abc{PHYLO_B}) and ($phylo_abc{PHYLO_A} + $phylo_abc{PHYLO_C})) {  #  sum of each side must be non-zero
                        $phylo_sorenson = sprintf("%.6f", eval {1 - (2 * $phylo_abc{PHYLO_A} / ($phylo_abc{PHYLO_A} + $phylo_abc{PHYLO_ABC}))});
                        $dist_result = $phylo_sorenson;
                    };
                # calculate the Sørensen distance                    
                } elsif ($measure == 1) {  # sorenson
                    ...
                    if (($abc{A} + $abc{B}) and ($abc{A} + $abc{C})) {  #  sum of each side must be non-zero
                        $sorenson = sprintf("%.6f", eval {1 - (2 * $abc{A} / ($abc{A} + $abc{ABC}))});
                        $dist_result = $sorenson;
                    };   
                };
                
                if (defined $dist_result) {
                    ...
                    if ($one_quota_region) { #1st of two alternative methods for managing the spread of distance values
                        ...
                        if ($dist_result == 1){    
                            ...
                            for my $bin_number (1..($bin_count-1)) {
                                if ($dist_result < $bins[$bin_number][0]) {
                    ...
                    if (!$skip) { 
                        $output_row = "$coords1[0],$coords1[1],$coords2[0],$coords2[1],$dist_result".$geog_dist_output.$regions_output."\n";
